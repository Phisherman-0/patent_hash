...
import { Search, Shield, CheckCircle, AlertCircle, ExternalLink, Calendar, User, FileText } from "lucide-react";
  ...
  const verificationMutation = useMutation({
    mutationFn: async (data: VerificationForm) => {
      if (data.type === "patent_id") {
        ...
        const response = await apiRequest(`/api/blockchain/verify/${data.value}`, {
          method: 'GET',
        });
        return { type: 'blockchain', data: response };
      } else if (data.type === "patent_number") {
        // Search for patent by patent number
        const response = await apiRequest(`/api/search/patents?q=${encodeURIComponent(data.value)}`, {
          method: 'GET',
        });
            ...
            {patents && patents.length > 0 ? (
              <div className="space-y-4">
                {patents.map((patent: any) => (
                  <Card key={patent.id} variant="outline">
